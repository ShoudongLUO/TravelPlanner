...
type Tab = 'itinerary' | 'accommodation' | 'budget' | 'tips' | 'videos' | 'feedback'
...
export default function ItineraryDetail({ itinerary }: Props) {
  ...
  const [review] = useState<ItineraryReview | null>(
    itinerary.itinerary_reviews?.[0] ?? null
  )
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: 'tips', label: '💡 贴士' },
    { key: 'videos', label: '📹 视频' },
    { key: 'feedback', label: review ? '📝 反馈 ⭐' : '📝 反馈' },
  ]
        ...
        <div className="p-6">
          ...
          {tab === 'feedback' && (
            <FeedbackTab
              ...
              onWriteFeedback={() => setShowReviewModal(true)}
            />
          )}
        </div>
      </div>
